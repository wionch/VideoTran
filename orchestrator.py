...
class Orchestrator:
    ...
    def run(self, video_path: str, src_lang: str, target_lang: str):
        """
        执行端到端的视频翻译流程。
        ...
        work_dir = tempfile.mkdtemp(prefix=f"videotran_{video_name}_{timestamp}_", dir=workspace_dir)
    ...
    def _get_t2_command(self, vocals_audio, src_lang, transcribed_json, config_path):
        cmd_prefix = self._get_command_prefix('transcriber')
        script_path = os.path.join(self.project_root, 'video_tran', 'transcriber', 'run.py')
        return f"{cmd_prefix} && python {script_path} --input-audio \"{vocals_audio}\" --lang {src_lang} --output-json \"{transcribed_json}\" --config-path \"{config_path}\""
    ...
    def _get_t4_command(self, translated_json, vocals_audio, dubbed_vocals_audio, temp_dir, target_lang):
        ...
        return f"{cmd_prefix} && python {script_path} --input-json \"{translated_json}\" --ref-audio \"{vocals_audio}\" --output-audio \"{dubbed_vocals_audio}\" --temp-dir \"{temp_dir}\" --target-lang \"{target_lang}\""

    def _get_t5_command(self, original_video, dubbed_vocals, bg_audio, segments_json, output_video, output_srt, temp_dir):
        cmd_prefix = self._get_command_prefix('main')
        script_path = os.path.join(self.project_root, 'video_tran', 'video_producer', 'run.py')
        return f"{cmd_prefix} && python {script_path} --original-video \"{original_video}\" --dubbed-audio \"{dubbed_vocals}\" --bg-audio \"{bg_audio}\" --segments-json \"{segments_json}\" --output-video \"{output_video}\" --output-srt \"{output_srt}\" --temp-dir \"{temp_dir}\""